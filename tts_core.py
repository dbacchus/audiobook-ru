...
# Перед именем бывает звание или родство со строчной буквы: «сказал отец
# Савва», «ответила баба Настя». Без этого имя не находится вовсе, и
# персонажа в книге просто нет (отец Савва, замечено Денисом 2026-09-21).
TITLE_WORD = (r"отец|батюшка|матушка|дядя|тётя|тетя|дед|дедушка|бабка|баба|"
              r"бабушка|мать|брат|сестра|доктор|товарищ|господин|госпожа|"
              r"старик|старуха|капитан|майор|тесть|кум")
# Между глаголом и именем бывает обстоятельство: «спросила на крыльце
# Аграфена Тимофеевна». Пропускаем до двух строчных слов, но НЕ
# местоимение: в «сказал он тихо Матвею» после глагола стоит настоящее
# подлежащее «он», а Матвей -- это тот, кому говорят.
_NOT_PRONOUN = r"(?!(?:он|она|они|ему|ей|им|их|себе|мне|нам|вам)\s)"
_NAME_AFTER_VERB = re.compile(
    rf"(?:{SPEECH_VERB})\s+(?:{_NOT_PRONOUN}[а-яё]+\s+){{0,2}}"
    rf"(?:(?:{TITLE_WORD})\s+)?((?:[А-ЯЁ][а-яё]+\s*){{1,3}})")
_NAME_BEFORE_VERB = re.compile(
    rf"(?:(?:{TITLE_WORD})\s+)?((?:[А-ЯЁ][а-яё]+\s*){{1,3}})(?:{SPEECH_VERB})")
...
def speaker_from_lead(text: str, cast, genders: dict = None,
    ...
    # а прочие имена в абзаце к нему отношения не имеют. И только в
    # именительном падеже: «голосом, похожим на голос Тихона... сказали:»
    # -- это не Тихон говорит, его имя тут в родительном.
    слева = []
    for name, варианты in _cast_forms(cast).items():
        for f in варианты:
            for m in re.finditer(rf"\b{re.escape(f)}\b",
                                 plain[:конец.start()], re.IGNORECASE):
...
# Кто действует в абзаце: имя или местоимение В НАЧАЛЕ ПРЕДЛОЖЕНИЯ ИЛИ
# ОБОРОТА. Просто «имя где-то в абзаце» не годится: в «и Тихон спросил,
# почему Кузьма голосовал против» последнее имя -- Кузьма, а действует Тихон.
_CLAUSE_HEAD = r"(?:^|(?<=[.!?;:])\s+|(?<=,)\s+(?:и\s+|а\s+|но\s+|потом\s+)?)"
# Оговорки: человек промолчал, не ответил или пересказан косвенной речью --
# значит следующую реплику произносит НЕ он.
_SILENT = re.compile(r"\bне\s+(?:ответи|сказа|отозва|возрази)\w*|"
                     r"\bмолча(?:л|ла|ли)\b", re.IGNORECASE)
_INDIRECT = re.compile(
    r"\b(?:сказа|говор|спроси|спрашива|ответи|отвеча|попроси|проси)\w*"
    r"(?:\s+\w+)?\s*,\s*(?:что|чтобы|почему|как|где|когда|зачем|куда|"
    r"кто|чем|нельзя|можно)\b|\bпопроси\w*\s+разрешения\b", re.IGNORECASE)


def speaker_from_action(narration: str, before: list, cast,
                        genders: dict = None, speech: str = ""):
    """Кто говорит, когда слов автора нет вовсе.

    «Она открыла корзину... В корзине лежали небольшие серые шары. -- Это
    глаза.» Реплика без слов автора идёт за абзацем повествования, и
    произносит её тот, кто в этом абзаце действовал (пример Дениса
    2026-09-21). Местоимение разрешается по предыдущим абзацам, как и в
    [[speaker_from_pronoun]]: только именительный падеж, буква в букву.

    Три оговорки, каждая из настоящей ошибки на повести:

    * «Тихон молчал.», «Матвей Ильич не ответил.» -- значит говорит другой;
    * «Тихон спросил, почему Кузьма голосовал против.» -- косвенная речь,
      и следующая реплика это ОТВЕТ, а не продолжение;
    * «Матвей Ильич не ответил. Он смотрел на камень. -- Матвей Ильич, я
      держу» -- к кому обращаются, тот и не говорит.

    Возвращает (имя, точно ли). Точно бывает только когда абзац кончается
    двоеточием: там сказано прямо, что дальше говорит он."""
    genders = genders or {}
    plain = unstress(narration or "").strip()
    if not plain:
        return None, False
    прямо = plain.endswith(":")
    формы = _cast_forms(cast)

    места = []
    for имя, ff in формы.items():
        for f in ff:
            for m in re.finditer(rf"{_CLAUSE_HEAD}{re.escape(f)}\b", plain):
                места.append((m.start(), имя, None))
    for m in re.finditer(rf"{_CLAUSE_HEAD}(он|она)\b", plain, re.IGNORECASE):
        места.append((m.start(), None, "м" if m.group(1).lower() == "он"
                      else "ж"))
    if not места:
        return None, False
    места.sort()
    ...
    # Оговорки смотрим в обороте, где этот деятель стоит
    хвост = plain[поз:]
    оборот = re.split(r"[.!?;]", хвост)[0]
    if _SILENT.search(оборот) or _INDIRECT.search(оборот):
        return None, False

    один = True
    if кто is None:                       # «Она» -- ищем имя раньше
        for текст in [plain[:поз]] + list(before or []):
            # По МЕСТУ В ТЕКСТЕ, а не по порядку в словаре: «Елена Карловна
            # ... Аня ... Она открыла корзину» -- ближайшая к местоимению
            # Елена, и перебор ролей в произвольном порядке давал Аню.
            подходят = []
            for имя, ff in формы.items():
                if genders.get(имя) != род:
                    continue
                for f in ff:
                    for m in re.finditer(rf"\b{re.escape(f)}\b",
                                         unstress(текст or ""), re.IGNORECASE):
                        подходят.append((m.start(), имя))
            if подходят:
                подходят.sort()
                кто = подходят[-1][1]
                один = len({и for _, и in подходят}) == 1
                break
        if кто is None:
            return None, False

    # К кому обращаются, тот не говорит
    речь = unstress(speech or "")
    for f in формы.get(кто, ()):
        if re.search(rf"\b{re.escape(_name_stem(f))}[а-яё]*\b", речь,
                     re.IGNORECASE):
            return None, False
    return кто, bool(прямо and один)
...
def dative_forms(name: str) -> set:
    """Дательный падеж имени: «Тихон» -> «Тихону», «Аня» -> «Ане».

    Морфологии тут не нужно: мы не разбираем слово, а СТРОИМ ожидаемую
    форму от именительного и ищем её в тексте. Лишние варианты никому не
    мешают -- они просто не встретятся."""
    n = name.lower()
    if n.endswith(("ский", "цкий", "ской")):
        return {name[:-2] + "ому"}
    if n.endswith("ия"):
        return {name[:-1] + "и"}
    if n.endswith(("а", "я")):
    ...
    if n.endswith(("й", "ь")):
        return {name[:-1] + "ю"}
    out = {name + "у"}
    if len(name) > 3 and n[-2] in "ео":      # беглая гласная: Павел -> Павлу
        out.add(name[:-2] + name[-1] + "у")
    return out


def addressee_from_author(chunk: str, cast) -> str | None:
    """Кому говорят: имя в дательном падеже в словах автора.

    «— Здесь Дальский, — сказала она Матвею Ильичу.» Это говорит сразу о
    двоих: Матвей не говорит эту реплику, а следующую, скорее всего,
    произносит он -- ему и отвечать (замечено Денисом 2026-09-21).

    Возвращает имя, только если адресат в куске один."""
    text = unstress(chunk)
    кому = set()
    for имя, формы in _cast_forms(cast).items():
        for f in формы:
            if any(re.search(rf"\b{re.escape(d)}\b", text, re.IGNORECASE)
                   for d in dative_forms(f)):
                кому.add(имя)
                break
    return кому.pop() if len(кому) == 1 else None
...
def speaker_from_author(chunk: str, cast) -> str | None:
    ...
    # то другое от запуска к запуску. Имя обязано стоять в ИМЕНИТЕЛЬНОМ:
    # подлежащее иначе не бывает, а «сказал он Тихону» иначе давало
    # Тихона -- да ещё с пометкой «точно» (замечено Денисом 2026-09-21).
    # «Матвею» не совпадало с «Матвей» по счастливой случайности русской
    # морфологии, а «Тихону» с «Тихон» совпадало.
    best = None
    for name, forms in _cast_forms(cast).items():
        for form in forms:
            hit = re.search(rf"\b{re.escape(form)}\b", hay)
...
# В многолюдной сцене чередование разрешено только в ДЛИННОЙ череде реплик.
# Измерено на спрятанных якорях: короткие череды там дают 6 верных ответов
# на 14 ошибок, а с порогом 6 многолюдные сцены перестают вредить вовсе.
ALT_MINRUN = 6
...
def _guess_scene(lines: list, cast: dict, known: dict = None,
    ...
    # Адресат в дательном падеже: «— Здесь Дальский, — сказала она Матвею
    # Ильичу». Отвечать будет Матвей, и это ближайшая следующая реплика --
    # если она идёт вплотную, без абзаца повествования между.
    кому_отвечать = {}
    for i in idx:
        куски = [c for k, c in split_reply(lines[i]) if k == "author"]
        if not куски:
            continue
        кому = addressee_from_author(" ".join(куски), cast)
        if кому and i + 1 < len(lines) and is_dialogue(lines[i + 1]):
            кому_отвечать[i + 1] = кому
    ...
    # Четвёртый источник: слов автора нет вовсе, и говорит тот, кто
    # действовал в предыдущем абзаце: «Она открыла корзину... — Это глаза»
    # (пример Дениса 2026-09-21). Правило точнее чередования, поэтому его
    # ответы помечаем и чередованием потом не перебиваем.
    по_действию = set()
    for i in idx:
        if out[i][1] == "точно" or i + сдвиг == 0:
            continue
        части = split_reply(lines[i])
        if [c for k, c in части if k == "author"]:
            continue
        пред = контекст[сдвиг + i - 1]
        if is_dialogue(пред):
            continue
        # Назад смотрим и в СЛОВА АВТОРА предыдущих реплик: «— сказала
        # Елена Карловна» -- такое же повествование, и «Она открыла
        # корзину» указывает именно туда, через реплику.
        n = сдвиг + i
        назад = []
        for j in range(n - 2, max(-1, n - 6), -1):
            стр = контекст[j]
            назад.append(" ".join(c for k, c in split_reply(стр)
                                  if k == "author") if is_dialogue(стр)
                         else стр)
        кто, точно = speaker_from_action(
            пред, назад, cast, genders or {},
            " ".join(c for k, c in части if k == "speech"))
        if кто:
            out[i] = (кто, "точно" if точно else "догадка")
            по_действию.add(i)
            if точно:
    ...
    # Пятый источник: род глагола. «— сказала она» не называет имени,
    ...
    # Шестой источник: тот, КОМУ говорили в предыдущей реплике. Ставим
    # после явных правил -- они надёжнее, -- но чередованию перебить не
    # даём: адресат назван в тексте, а чередование только считает.
    for i, кому in кому_отвечать.items():
        if out[i][1] != "точно":
            out[i] = (кому, "догадка")
            по_действию.add(i)
    ...
    if not anchors:
        return out
    scene_voices = list(dict.fromkeys(anchors.values()))
    чиста = len(scene_voices) == 2      # во всей сцене названы ровно двое
    ...
    for череда in череды:
        ...
        if not свои:
            continue
        # «Ровно двое» считается ПО ЧЕРЕДЕ, а не по всей сцене. Сцена
        # бывает длинной и многолюдной -- Аня с Тихоном говорят в одном её
        # конце, Кузьма с Еленой в другом, -- и общий счёт по сцене
        # выключал чередование там, где на самом деле разговор двоих
        # (замечено Денисом 2026-09-21 на разговоре про Паромщика).
        имена = list(dict.fromkeys(свои.values()))
        # Самопроверка череды: если между двумя якорями говорил кто-то
        # третий, не названный по имени, чётность между ними уже не
        # сходится. Требуем, чтобы у ВСЕХ пар якорей чётное расстояние
        # означало одно имя, а нечётное -- разные имена.
        согласны = all(((abs(i - j) % 2 == 0) == (свои[i] == свои[j]))
                       for i in свои for j in свои if i < j)
        if not согласны:
            continue
        # В ЧИСТОЙ сцене (по именам во всей сцене говорят ровно двое)
        # хватает одного якоря на череду: третьему взяться неоткуда. В
        # многолюдной сцене одного якоря мало -- нужны хотя бы два, иначе
        # проверка чётности ничего не проверяет.
        if чиста and len(имена) == 1:
            имена = list(scene_voices)
        elif not чиста and (len(свои) < 2 or len(череда) < ALT_MINRUN):
            continue
        # Второго собеседника автор называет не всегда: «Первые недели
        # Тихон спрашивал... — объяснила Аня... — сказала Аня». Имя есть
        # только у Ани, Тихон живёт в повествовании перед чередой. Берём
        # его оттуда, но только если там назван РОВНО ОДИН другой человек.
        if len(имена) == 1:
            начало = сдвиг + череда[0]
            рассказ = " ".join(
                unstress(контекст[j] or "")
                for j in range(max(0, начало - 3), начало)
                if not is_dialogue(контекст[j]))
            прочие = {имя for имя, ff in _cast_forms(cast).items()
                      if имя != имена[0]
                      and any(re.search(rf"\b{re.escape(f)}\b", рассказ,
                                        re.IGNORECASE) for f in ff)}
            if len(прочие) == 1:
                имена = имена + list(прочие)
        if len(имена) != 2:
            continue
        a, b = имена
        other = {a: b, b: a}
