            ...
            for n, p in enumerate(files, 1):
                ...
                for title, items in sections:
                    ...
                    if roles is not None:
                        ...
                        # ПАРЫ, а не голые строки: по видам («brk», «title»)
                        # разбор режет главу на сцены, а чередование работает
                        # только внутри сцены. С голыми строками вся глава
                        # была одной сценой, говорящих в ней больше двух,
                        # и чередование выключалось -- таблица показывала
                        # одно, а звучало другое.
                        got = core.guess_speakers(items, roles["voices"],
                                                  roles.get("speakers"),
                                                  roles.get("genders"))
...
def check_layout(log) -> bool:
    """Не срезано ли что-нибудь по ширине в окне.

    Ряд виджетов, разложенных вбок (pack side=left/right), может не влезть
    в отведённую ширину, и тогда крайние просто уезжают за край. В журнале
    этого не видно, на глаз замечается не сразу, а случалось уже трижды:
    строка состояния, поле «Книга / автор», строка фильтров.

    Проверка обходит все три вкладки и сравнивает сумму нужных ширин детей
    с шириной самого ряда. Ряды, где дети растягиваются (expand), не в
    счёт -- они и рассчитаны на сжатие."""
    try:
        win = App()
        win.update()
        # Размер берём ИСХОДНЫЙ, а не развёрнутый: на большом экране в
        # растянутое окно влезает и сломанный ряд, и проверка молчит --
        # именно так она проспала первую подсунутую ей поломку.
        win.update_idletasks()
        win.update()
    except Exception as e:
        log(f"раскладка: проверить не удалось ({e})")
        return True

    беды = {}

    def обойти(w):
        дети = w.winfo_children()
        вбок = [c for c in дети
                if c.winfo_manager() == "pack"
                and c.pack_info().get("side") in ("left", "right")]
        if len(вбок) >= 2 and not any(
                str(c.pack_info().get("expand")) in ("1", "True") for c in вбок):
            def отступ(c):
                # padx бывает числом и парой «слева, справа»
                p = c.pack_info().get("padx", 0) or 0
                if isinstance(p, (tuple, list)):
                    return sum(int(x) for x in p)
                return int(p) * 2

            нужно = sum(c.winfo_reqwidth() + отступ(c) for c in вбок)
            есть = w.winfo_width()
            if есть > 1 and нужно > есть:
                # подписи собираем СРАЗУ: к моменту отчёта окно уже
                # уничтожено, и спрашивать у виджетов будет некого
                подписи = []
                for c in вбок:
                    try:
                        t = c.cget("text")
                        ...
                        t = ""
                    if t:
                        подписи.append(str(t)[:18])
                # окно обходится на каждой вкладке, и один ряд попадался бы
                # несколько раз -- запоминаем по самому виджету
                беды[str(w)] = (подписи, нужно, есть)
        for c in дети:
            обойти(c)

    try:
        for i in range(len(win.nb.tabs())):
            win.nb.select(i)
            win.update_idletasks()
            win.update()
            обойти(win)
    ...
    if not беды:
        log("раскладка: всё влезает по ширине")
        return True
    for подписи, нужно, есть in list(беды.values())[:6]:
        log(f"ОШИБКА раскладки: ряду нужно {нужно} точек, есть {есть} — "
            + ", ".join(подписи[:5]))
    return False
...
def run_selftest():
    ...
    try:
        ...
        # Аудиоспектакль: чередование говорящих работает только внутри
        # сцены, а границы сцен видны лишь в ПАРАХ (вид, текст). Дважды
        # уже выходило, что таблица показывает одно, а звучит другое, --
        # поэтому проверяем именно то, чем отличается путь синтеза.
        сцена = [("par", "— Ты оп+ять зд+есь ночев+ал, — сказ+ала +Аня."),
                 ("par", "— Я не ночев+ал. Я раб+отал."),
                 ("par", "— Д+ед. Доск+а т+а же."),
                 ("par", "— Доск+а т+а же, — соглас+ился Матв+ей.")]
        got = core.guess_speakers(сцена, {"Аня", "Матвей"})
        кто = [g[0] for g in got]
        if кто != ["Аня", "Матвей", "Аня", "Матвей"]:
            log(f"ОШИБКА: чередование говорящих -- {кто}")
            ok = False

        # «— сказал он»: имя берётся из повествования ПЕРЕД сценой, и
        # только в именительном падеже. Тут проверяются обе половины:
        # Кузьму надо найти через границу сцены (два абзаца повествования
        # сцену обрывают), а «отдал Тихону колесо» -- это адресат, и
        # говорит не он. Одно слово в падеже -- и правило врёт уверенно.
        мест = [("par", "Первым увидел Кузьма. Он всю жизнь смотрел на воду."),
                ("par", "Вода шла мутная, с верховьев."),
                ("par", "— Ильич, — сказал он. — Глянь."),
                ("par", "Матвей Ильич отдал Тихону колесо и встал рядом."),
                ("par", "— Держи, — сказал он. — На месте.")]
        got = core.guess_speakers(мест, {"Кузьма", "Матвей", "Тихон"},
                                  genders={"Кузьма": "м", "Матвей": "м",
                                           "Тихон": "м"})
        кто = [g[0] for g in got if g]
        if кто != ["Кузьма", "Матвей"]:
            log(f"ОШИБКА: «сказал он» -- {кто}, ждали Кузьма и Матвей")
            ok = False

        # Дательный падеж в словах автора: «сказал он Тихону» -- это КОМУ
        # говорят, а не кто говорит. Раньше это давало Тихона говорящим, да
        # ещё с пометкой «точно»: «Матвею» не совпадало с «Матвей» по
        # случайности, а «Тихону» с «Тихон» совпадало. Винительный
        # («про Тихона») адресатом быть не должен.
        двое = {"Тихон", "Аня"}
        if (core.speaker_from_author("сказал он Тихону", двое) is not None
                or core.addressee_from_author("сказал он Тихону",
                                              двое) != "Тихон"
                or core.addressee_from_author("спросил он про Тихона",
                                              двое) is not None):
            log("ОШИБКА: дательный падеж в словах автора")
            ok = False
        пара2 = [("par", "— Здесь Дальский, — сказала она Тихону."),
                 ("par", "— Кто это?")]
        got = core.guess_speakers(пара2, двое)
        if got[1][0] != "Тихон":
            log(f"ОШИБКА: отвечает тот, кому говорили -- {got}")
            ok = False

        # Реплика вовсе без слов автора: говорит тот, кто действовал в
        # предыдущем абзаце. «Она» тут указывает в слова автора прошлой
        # реплики -- через реплику, а не только по повествованию. Вторая
        # половина проверки -- оговорка: после «Тихон молчал» говорит
        # ДРУГОЙ, и правило обязано промолчать.
        дел = [("par", "— Здесь будет только он, — сказала Елена Карловна."),
               ("par", "Она открыла корзину, которую несла с собой. "
                       "В корзине лежали серые шары."),
               ("par", "— Это глаза. Внутри камера, микрофон и голос."),
               ("par", "Тихон молчал."),
               ("par", "— Ты чего?")]
        got = core.guess_speakers(дел, {"Елена", "Тихон"},
                                  genders={"Елена": "ж", "Тихон": "м"})
        кто = [g[0] for g in got if g]
        if кто != ["Елена", "Елена", None]:
            log(f"ОШИБКА: реплика без слов автора -- {кто}, "
                f"ждали Елену дважды и молчание")
            ok = False

        # Имя со званием или родством: «сказал отец Савва». Имя ищется как
        # слово с большой буквы сразу за глаголом речи, и из-за строчного
        # «отец» персонажа в книге не было вовсе -- его реплики доставались
        # соседям (замечено Денисом 2026-09-21).
        # Вторая половина: между глаголом и именем бывает обстоятельство
        # («спросила на крыльце Аграфена Тимофеевна»), и это тоже прятало
        # человека целиком.
        поп = ["Отец Савва пришёл к вечеру. Аграфена Тимофеевна сидела рядом.",
               "— Для того, кто далеко, — сказал отец Савва.",
               "— Ну и хорошо, — сказал отец Савва. — Один раз можно.",
               "— Это он? — спросила на крыльце Аграфена Тимофеевна."]
        роли = core.detect_characters(поп)
        if роли.get("Савва") != 2 or роли.get("Аграфена") != 1:
            log(f"ОШИБКА: имя со званием или через обстоятельство -- {роли}")
            ok = False

        # Разговор двоих внутри многолюдной сцены: имя названо только у
        # Ани, второго собеседника автор называет лишь в повествовании
        # перед чередой реплик. Проверяются обе половины: партнёр берётся
        # из повествования, а чётность считается внутри череды.
        пара = [("par", "Первые недели Тихон спрашивал."),
                ("par", "Спрашивал он обо всём, и первый его вопрос Ане "
                        "был про паром."),
                ("par", "— Потому что раньше был паромщик, — объяснила Аня."),
                ("par", "— То есть паром получил имя от человека."),
                ("par", "— Да."),
                ("par", "— И он не против?"),
                ("par", "— Паром?"),
                ("par", "— Дядя Гриша."),
                ("par", "— Дядя Гриша умер, — сказала Аня.")]
        got = core.guess_speakers(пара, {"Аня", "Тихон"},
                                  genders={"Аня": "ж", "Тихон": "м"})
        кто = [g[0] for g in got if g]
        ждём = ["Аня", "Тихон", "Аня", "Тихон", "Аня", "Тихон", "Аня"]
        if кто != ждём:
            log(f"ОШИБКА: собеседник из повествования -- {кто}")
            ok = False
        ...
        # Раскладка окна: ряды, где детям не хватает ширины. Такое трижды
        # уходило в сборку незамеченным (строка состояния, поле «Книга /
        # автор», строка фильтров): в журнале ничего не видно, ловится
        # только измерением.
        if not check_layout(log):
            ok = False
    except Exception:
        ok = False
